...
def make_quantum_teleportation_circuit(ranX, ranY):
    ...
    #Uses the two classical bits from the Bell measurement to recover the original quatum message on Bob's entangled qubit
...
def main():
    #Encode random state to teleport
    ranX = random.random()
    ranY = random.random()
    ...
    b0X, b0Y, b0Z = cirq.bloch_vector_from_state_vector(message.state_vector(), 0)
    print("x: ", round(b0X, 4), "y: ", round(b0Y, 4), "z: ", round(b0Z, 4))

    #Display the quantum teleportation circuit
    print("\nCircuit: ")
    print(circuit)
    ...
    b2X, b2Y, b2Z = cirq.bloch_vector_from_state_vector(final_results.state_vector(), 2)
